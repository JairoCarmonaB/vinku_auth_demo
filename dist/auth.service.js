"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AuthService = void 0;
const common_1 = require("@nestjs/common");
const config_module_definition_1 = require("./config.module-definition");
const firebase_provider_1 = require("./providers/firebase.provider");
let AuthService = class AuthService {
    constructor(authentication, options) {
        this.authentication = authentication;
        this.options = options;
        if (!(options === null || options === void 0 ? void 0 : options.file))
            throw new common_1.NotFoundException(`Its necessary a config file to auth in vinku_auth`);
        this.authentication.initializeApp(options.file);
    }
    loggedByToken(token) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.authentication.loggedByToken(token);
        });
    }
    createSessionToken(uid) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.authentication.createSessionToken(uid);
        });
    }
    getAccount(email) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.authentication.getAccount(email);
        });
    }
    createAccount(accountDetails) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.authentication.createAccount(accountDetails);
        });
    }
    deleteAccount(uid) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.authentication.deleteAccount(uid);
        });
    }
};
AuthService = __decorate([
    (0, common_1.Injectable)(),
    __param(1, (0, common_1.Inject)(config_module_definition_1.MODULE_OPTIONS_TOKEN)),
    __metadata("design:paramtypes", [firebase_provider_1.FirebaseProvider, Object])
], AuthService);
exports.AuthService = AuthService;
